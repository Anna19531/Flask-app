...
from wtforms import DateField, EmailField, PasswordField
from wtforms.validators import DataRequired


# form to add events
class EventForm(FlaskForm):
    name = StringField("Event name", validators=[validators.Length(
        min=1, max=144), validators.DataRequired()])
...
class TaskForm(FlaskForm):
    ...
    hours = IntegerField("Hours it will take", validators=[
        validators.NumberRange(min=0, max=24), validators.DataRequired()])
...
def taskForm(request):
    ...
    form.event.choices = [
        ((event.id, event.name) for event in Event.query.all()), (0, "Other")]
...
class TodayForm(FlaskForm):
    ...
    personal = IntegerField("Personal", [
